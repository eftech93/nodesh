...
import { IntelligentCompleter, createCompleter, addIntrospectionMethods } from '../src/autocomplete';
import { ObjectMetadata, PropertyInfo } from '../src/types';
...
class UserService {
  ...
  async findByEmail(email: string) { return { email }; }
  async findActive(options = {}) { return []; }
  async update(id: string, updates: Record<string, unknown>) { return { id, ...updates }; }
  async delete(id: string) { return { id, deleted: true }; }
  async authenticate(email: string, password: string) { return null; }
...
class OrderService {
  ...
  async create(userId: string, items: unknown[]) { return { id: 1, userId, items }; }
  async findById(id: string) { return { id }; }
  async findByUserId(userId: string) { return []; }
  async updateStatus(id: string, status: string) { return { id, status }; }
  async cancel(id: string) { return { id, cancelled: true }; }
}
...
describe('IntelligentCompleter', () => {
  let completer: IntelligentCompleter;
  ...
  beforeEach(() => {
    ...
    completer = new IntelligentCompleter(context);
  });
...
describe('createCompleter', () => {
  it('should return a completer function', () => {
    ...
    const completerFn = createCompleter(context);
...
describe('addIntrospectionMethods', () => {
  ...
  it('info() should return metadata for string key', () => {
    const metadata = (context.info as Function)('userService');
  ...
  it('info() should return metadata for object', () => {
    const metadata = (context.info as Function)(context.userService);
  ...
  it('methods() should list all methods', () => {
    const methods = (context.methods as Function)('userService');
  ...
  it('props() should list all non-method properties', () => {
    const props = (context.props as Function)('config');
  ...
  it('type() should return type for string key', () => {
    const type = (context.type as Function)('userService');
    
    expect(type).toBe('UserService');
  });

  it('type() should return type for value', () => {
    const type = (context.type as Function)(123);
